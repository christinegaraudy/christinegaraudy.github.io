...
// 0. Assignment operators: Assign values to variables.
    ...
    /* F) %= gets the remainder/modulus of left operand divided by right and assigns it 
            to the left operand */
...
// 1. Arithmetic operators:
    ...
    /* E) % modulus: gets the remainder(modulus) created from dividing right operand 
        from left
    
    /* F) ++ incrementor: increases operand value by 1 commonly used for looping 
        forwards over an array. */
    
    /* G) -- decrementor: decreases operand value by 1. commonly used for looping 
        backwards over an array. */
    ...
    11 % 2; /* divides 11 by 2, returns the remainder of 1 (2 goes into 11 5 times, 
                with 1 left over) */
    c++; // returns 4 (3 + 1)
    d--; // returns 4 (5 - 1)
...
// 2.Comparison operators: Compare two values and return an evaluation of true or false.
    ...
    /* A) == loosely equal to (isn't concerned with type, so the string "2" would 
        evaluate as loosely equal to the number 2, for example) /
    
    /* B) === strictly equal to (is concerned with type, so "2" would not evaluate as 
        strictly equal to 2) */
    
    /* C) != not loosely equal to (there is also !== not strictly equal to, which works 
        as the inverse of the example above) */
    ...
    /* F) >= greater than or equal to (is the left operand larger or the same as the 
        one on the right?) */
    
    /* G) <= less than or equal to (is the left operand smaller or the same as the 
        one on the right?) */
...
// 3. Logical operators: Combine and compare two conditions
    ...
    // A) && and: the statement on the left AND the statement on the right are truthy 
    ...
    !(2 > 1); /* returns false because (2 > 1) is true and the bang (!) operator 
                reserves that value to make it become false */
...
/* 5. Ternary operator (a ? b : c): Accepts three operands. It begins with a condition, 
    then a question mark (?). Next, there are two expressions separated by a colon (:).  
    If the condition evaluates to be true, the first expression will be executed.  If 
    the condition evaluates as false, the second expression will be executed. 
    (condition ? expression1 : expression2)  Useful as a way to more cleanly condense 
    an if-else statement (if condition is true, execute expression1, else execute 
    expression2) */
    
    var e = 7, f = 8;
    (e > f) ? 'yes' : 'no'; /* returns 'no' because conditional statement evaluates to 
                                false, so second expression executes */